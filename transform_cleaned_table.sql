...
SELECT
    ...
    PARSE_DATE('%Y-%m-%d', start_date_local) AS local_start_date,
    PARSE_TIME('%H:%M:%S', start_date_local) AS local_start_time,
    timezone,
    SUBSTR(timezone, STRPOS(timezone, ')') + 2) AS timezone_name,
    kudos_count,
    start_latlng,
    end_latlng,
    TRIM(REPLACE(SUBSTR(start_latlng, 2, STRPOS(start_latlng, ',') - 1), ']', '')) AS start_latitude,
    TRIM(REPLACE(SUBSTR(start_latlng, STRPOS(start_latlng, ',') + 2), ']', '')) AS start_longitude,
    TRIM(REPLACE(SUBSTR(end_latlng, 2, STRPOS(end_latlng, ',') - 1), ']', '')) AS end_latitude,
    TRIM(REPLACE(SUBSTR(end_latlng, STRPOS(end_latlng, ',') + 2), ']', '')) AS end_longitude,
