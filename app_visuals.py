...
def make_bq_client():
    # Streamlit Community Cloud: put your service account JSON in st.secrets["gcp_service_account"]
...
sport = st.sidebar.selectbox("Sport", ["(All)"] + sports)
...
def load_data(sport, cities, start_d, end_d):
    params = [
        ...
        bigquery.ScalarQueryParameter("sport", "STRING", "" if sport == "(All)" else sport),
    ]
    cities_cond = "TRUE"
    if cities:
...
c2.metric("Total miles", round(df["distance_miles"].sum(), 1) if not df.empty else 0)
c3.metric("Median pace (min/mi)", round(df["pace_min_per_mile"].median(), 2) if not df.empty else 0)
c4.metric("Total elevation (ft)", int(df["elevation_gain"].sum()) if not df.empty else 0)
...
if df.empty:
    ...
    with t2:
        pace_by_day = df.groupby("d", as_index=False)["pace_min_per_mile"].median()
    ...
    with b1:
        ...
        by_city = df.groupby("city", as_index=False)["distance_miles"].sum().sort_values("distance_miles", ascending=False).head(10)
    ...
    map_df = df.dropna(subset=["start_latitude", "start_longitude"])[["start_latitude", "start_longitude"]]
    map_df = map_df.rename(columns={"start_latitude": "lat", "start_longitude": "lon"})
    if not map_df.empty:
    ...
    st.dataframe(
        df[["d", "subtype", "city", "name", "distance_miles", "pace_min_per_mile", "elevation_gain"]],
